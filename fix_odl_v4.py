#!/usr/bin/env python3
"""Create ODL file using BASIC with more robust waiting."""
...
def main():
    ...
    # Create ODL using BASIC with stored program
    print("\n=== Creating ODL file with BASIC ===")
    child.sendline('RUN $BP2IC2')
    child.expect('BASIC2', timeout=15)

    # Enter program lines - use simpler pattern matching
    def basic_line(line):
        child.sendline(line)
        time.sleep(0.5)  # Give time for echo and processing
    ...
    # Give time for all lines to process
    time.sleep(1)

    # List the program to verify
    child.sendline('LIST')
    child.expect('BASIC2', timeout=15)
    ...
    child.sendline('RUN')
    idx = child.expect(['BASIC2', r'\?', 'Ready'], timeout=30)
    if idx == 1:
        print("\n*** BASIC RUN ERROR ***")
        # Try to get back to prompt
        child.expect(['BASIC2', r'\$ '], timeout=15)
