#!/usr/bin/env python3
"""Rebuild ADVINI with fixed device reference (_DK1: -> _DM1:) - corrected compile syntax."""
...
def main():
    print("=" * 60)
    print("Rebuild ADVINI with _DM1: device (correct compile syntax)")
    print("=" * 60)
    ...
    while login_attempts < 10:
        ...
        if idx == 0:
            ...
            print("\n*** Logins disabled - waiting and retrying ***")
            time.sleep(10)
            child.sendline('')
        elif idx == 5:
            child.sendline('')
    ...
    # Compile ADVINI.SUB - CORRECT SYNTAX: COMPILE/BP2
    ...
    if idx == 0:
        ...
        child.sendline('LOOK')
        time.sleep(3)
        idx2 = child.expect(['#', 'What', 'room', 'Room', r'\$ ', pexpect.TIMEOUT], timeout=15)
