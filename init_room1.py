#!/usr/bin/env python3
"""Initialize Room 1 with valid data so game can start."""
...
def main():
    print("=" * 60)
    print("Initialize Room 1 with Valid Data")
    print("=" * 60)
    ...
    # Create program to initialize room 1
    # Based on ADVMSG.SUB: FIELD #3%, 1% AS ROOM$, 16% AS EX$, 83% AS PEOPLE$, 100% AS OBJECT$, 312% AS DESC$
    lines = [
        '10 EXTEND',
        ...
        '200 GET #1%, RECORD 1%',
        '210 LSET ROOM$=CHR$(1%)',
        # Exits: N0001S0001E0001W0001 (all lead to room 1 for now)
        '220 LSET EX$="N0001S0001E0001W0001"',
        # Initialize PEOPLE$ - empty
        '230 LSET PEOPLE$=STRING$(83%,0%)',
        # Initialize OBJECT$ - empty
        '240 LSET OBJECT$=STRING$(100%,0%)',
        # Room description
        '250 D$="You are standing in a vast underground cavern.'+chr(10)+'Passages lead in all directions."',
        '260 LSET DESC$=D$+STRING$(312%-LEN(D$),0%)',
        '270 PUT #1%, RECORD 1%',
        '280 PRINT "Room 1 initialized"',
        # Initialize ADVENT.CHR character 1 as empty but valid
        '300 CLOSE #1%',
        ...
        # Initialize ADVENT.MON monster 1 as empty
    ...
    if idx in [0, 1, 2, 3, 4, 7]:
        ...
        # Check result
        try:
            idx3 = child.expect(['Password', '>', 'Welcome', 'cave', r'\?', pexpect.TIMEOUT], timeout=30)
            print(f"\n*** Third expect: {idx3} ***")
            if idx3 == 0:
                # Password prompt
                child.sendline('HERO')
                time.sleep(3)
                try:
                    idx4 = child.expect(['>', 'Welcome', 'cave', r'\?', pexpect.TIMEOUT], timeout=30)
                    print(f"\n*** Fourth expect: {idx4} ***")
                except:
                    pass
        except:
