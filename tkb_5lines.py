#!/usr/bin/env python3
"""Build ADVENT.TSK - exactly 5 continuation lines (under TKB limit)."""
...
def main():
    print("=" * 60)
    print("Build ADVENT.TSK - 5 continuation lines")
    print("=" * 60)
    ...
    # Run TKB interactively with exactly 5 continuation lines
    print("\n=== Running TKB (5 continuation lines) ===")
    child.sendline('RUN $TKB')
    child.expect('TKB>', timeout=15)
    time.sleep(1)

    # Line 1: Output, map, main + 3 subs = 4 files
    tkb_lines = [
        'DM1:[1,2]ADVENT/FP,DM1:[1,2]ADVENT=DM1:[1,2]ADVENT,DM1:[1,2]ADVINI,DM1:[1,2]ADVOUT,DM1:[1,2]ADVNOR/-',
        # Line 2: 4 subs
        'DM1:[1,2]ADVCMD,DM1:[1,2]ADVODD,DM1:[1,2]ADVMSG,DM1:[1,2]ADVBYE/-',
        # Line 3: 4 subs
        'DM1:[1,2]ADVSHT,DM1:[1,2]ADVNPC,DM1:[1,2]ADVPUZ,DM1:[1,2]ADVDSP/-',
        # Line 4: 2 subs + library (no more continuation)
        'DM1:[1,2]ADVFND,DM1:[1,2]ADVTDY,SY:[1,1]BP2OTS/LB',
    ]

    for i, line in enumerate(tkb_lines):
        print(f"\n>>> [{i+1}/{len(tkb_lines)}] {line[:50]}...")
        child.sendline(line)
        time.sleep(2)
