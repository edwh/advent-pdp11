#!/usr/bin/env python3
"""Create data files using a proper BASIC program - fixed."""
...
def main():
    print("=" * 60)
    print("Create Data Files Using BASIC Program V4")
    print("=" * 60)
    ...
    # Enter program lines directly (no NEW needed if we just enter line numbers)
    program_lines = [
        '10 EXTEND',
    ...
    # Make sure we're at BASIC2 prompt
    ...
    idx = child.expect(['ALL FILES CREATED', 'Error', 'BASIC2', 'done', pexpect.TIMEOUT], timeout=180)
    print(f"\n*** Program execution result: {idx} ***")

    # If we saw partial output, continue waiting
    if idx == 3:  # saw 'done'
        idx = child.expect(['ALL FILES CREATED', 'Error', 'BASIC2', pexpect.TIMEOUT], timeout=120)
        print(f"\n*** Final result: {idx} ***")
