#!/usr/bin/env python3
"""Rebuild ADVINI with fixed device reference - using BASIC/BP2 command."""
...
def main():
    print("=" * 60)
    print("Rebuild ADVINI with _DM1: device (using BASIC/BP2)")
    print("=" * 60)
    ...
    def dcl_cmd(cmd, timeout=30):
        ...
        child.sendline(cmd)
        while True:
            idx = child.expect([r'\$ ', 'Topic', 'Subtopic', pexpect.TIMEOUT], timeout=timeout)
            if idx == 0:
                break
            elif idx in [1, 2]:
                child.sendcontrol('z')
    ...
    # Compile ADVINI.SUB using BASIC/BP2 command
    print("\n=== Compiling ADVINI.SUB with BASIC/BP2 ===")
    child.sendline('BASIC/BP2 A:ADVINI.SUB')
    time.sleep(5)

    # Wait for compilation to complete
    idx = child.expect([r'\$ ', 'Error', 'error', '%', 'Ready', pexpect.TIMEOUT], timeout=120)
    print(f"\n*** Compile result: idx={idx} ***")

    if idx in [1, 2, 3]:
        # Error occurred - wait for prompt
        child.expect([r'\$ ', pexpect.TIMEOUT], timeout=30)
    elif idx == 4:
        # Got Ready prompt - exit BASIC
        child.sendline('$')
        child.expect([r'\$ ', pexpect.TIMEOUT], timeout=15)
    ...
    if idx == 0:
        ...
        if idx2 in [0, 2, 3, 4]:
            # Got a response - game is working!
            print("\n*** SUCCESS! Game commands are working! ***")
            child.sendline('HELP')
            time.sleep(3)
            child.expect(['#', r'\$ ', pexpect.TIMEOUT], timeout=15)
